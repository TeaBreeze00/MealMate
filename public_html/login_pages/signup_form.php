...
function generateVendorId() {
    ...
    if ($result) {
        ...
        return ($row['maxID'] ?? 0) + 1; // Handle case where table is empty
    } else {
        ...
        if ($db_connected) {
            ...
            if ($statement) {
                ...
                if (mysqli_stmt_execute($statement)) {
                    ...
                    header("location: ../user_pages/vendor_homepage.php");
                } else {
...
<head>
    ...
    <title>MealMate - Vendor Registration</title>
    <style>
        body, html {
            margin: 0;
            padding: 0;
...
<div class="container">
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST" style="width: 100%; display: flex; flex-direction: column; align-items: center;">
        ...
        <input type="text" name="phoneNo" placeholder="Enter your phone number (required)" value="<?php echo htmlspecialchars($phoneNo ?? ''); ?>">
